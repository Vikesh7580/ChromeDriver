...
function getPageCache(opt_doc, opt_w3c) {
  ...
  var key = '$xyz_asdjflasutopfhvcZLmcfl_';
  if (w3c) {
    if (!(key in doc))
...
function isCollection(value) {
  const Symbol = window.xyz_adoQpoasnfa76pfcZLmcfl_Symbol || window.Symbol;
...
function cloneWithAlgorithm(item, seen, algo, opt_cache) {
  ...
  if (isCollection(item)) {
    const Array = window.xyz_adoQpoasnfa76pfcZLmcfl_Array || window.Array;
...
function callFunction(func, args, w3c, opt_unwrappedReturn) {
  ...
  let status = 0;
  let returnValue;
  const Promise = window.xyz_adoQpoasnfa76pfcZLmcfl_Promise || window.Promise;
  try {
